...
const ChatBox = () => {
  ...
  return (
    <div className="flex flex-col h-full text-white">
      <div className="flex-1 p-4 max-h-[calc(100vh-14rem)] overflow-y-auto scrollbar-hide flex flex-col">
        {Object.keys(groupedMessages).map((date, dateIndex) => (
          <div key={dateIndex} className="flex flex-col mb-4">
            <div className="text-center text-gray mb-4 text-xs">
            ...
            {groupedMessages[date].map((message, index) => {
              ...
              return (
                <div key={index} className="flex flex-col">
                  {showTimestamp && (
                    <div
                      className={`text-xs text-gray mb-2 ${
                        message.sender === "user"
                          ? "self-end text-right"
      ...
      <div className="p-2 bg-black rounded-xl flex flex-row gap-3 mt-4 items-center">
        <input
          type="text"
          className="w-full p-2 bg-black border-none outline-none rounded-xl text-white placeholder-gray"
